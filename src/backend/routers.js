...
export function sitemap_Router(request) {
   ...
   if (data) {
       ...
       // Render item page 
       return ok("sitemap-page", data, seoData); 
   }
...
export function sitemap_SiteMap(sitemapRequest) {
   ...
   const siteMapEntries = Object.keys(peopleData).map((name)=>{
                               ...
                               entry.pageName = "sitemap-page";		// The name of the page in the Wix editor to render
                               entry.url = "/sitemap/" + name ;			// Relative URL of the page
